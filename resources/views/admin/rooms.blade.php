...
@section('content')
    <div class="flex justify-between mb-6">
        ...
        <a href="/admin/rooms/create" class="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600">Tambah
            Kamar</a>
    </div>
                    ...
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="p-3 flex gap-2">
                            <a href="/admin/rooms/{{ $room->id }}/edit"
                                class="px-3 py-1 bg-blue-500 text-black rounded hover:bg-blue-600">Edit</a>
                            <form action="/admin/rooms/{{ $room->id }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button class="px-3 py-1 bg-red-500 text-black rounded hover:bg-red-600"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
        ...
        @foreach ($rooms as $room)
            <a href="{{ route('admin.rooms.edit', $room->id) }}"
                ...
                {{-- GAMBAR --}}
                <img src="{{ secure_asset('storage/' . $room->image) }}"
